...
var $author$project$Data$Likert$ratingsFromMedian = F2(
	function (count, median) {
		return A2($elm$core$List$range, median - ((count / 2) | 0), median + ((count / 2) | 0));
	});
var $author$project$Data$Likert$scale = F4(
	function (title, description, statements, labels) {
		var zip = F2(
			function (xs, ys) {
				return A3($elm$core$List$map2, $elm$core$Tuple$pair, xs, ys);
			});
		var ratingValues = A2(
			$author$project$Data$Likert$ratingsFromMedian,
			$elm$core$List$length(labels),
			0);
		return {
			description: description,
			ratings: A2(zip, ratingValues, labels),
			statements: A2(
				$elm$core$Basics$composeR,
				$elm$core$Set$toList,
				$elm$core$List$map(
					$elm$core$Tuple$pair(0)))(statements),
			title: title
		};
	});
var $author$project$Data$Likert$fivePointScale = F3(
	function (title, description, statements) {
		return A4(
			$author$project$Data$Likert$scale,
			title,
			description,
			statements,
			_List_fromArray(
...
var $author$project$Main$scaleOne = function () {
	var title = 'The extent to which the end-goal is defined.';
	var statements = $elm$core$Set$fromList(
		_List_fromArray(
			['Before starting to program I have a clear idea what the program will look like.', 'Before starting to program I have a clear idea how the program will work.', 'Before starting to program I have a clear idea how to organise my code.', 'Before starting to program I write down what I need to do.', 'Before starting to program I sketch or wireframe how the program will look.']));
	var description = 'Some programmers have a very clear idea of what the finished ' + ('piece of software should look like and what it should do. Others take a ' + ('more exploratory approach. The following statements are about how clearly ' + 'defined the end-goal of a project is before you start programming.'));
	return A3($author$project$Data$Likert$fivePointScale, title, description, statements);
}();
var $author$project$Main$scaleTwo = function () {
	var title = 'The extent of programming effort, over time, and in the size of the resulting codebase';
	var statements = $elm$core$Set$fromList(
		_List_fromArray(
			['A typical project takes a significant amount of time to complete.', 'A finished project’s codebase tends to span multiple files with many lines of code.', 'I tend to considered a project finished after one programming session.', 'A finished project’s codebase tends to be made up of one or two files.']));
	var description = 'Some programmers work on projects for many months or even years. ' + ('Others prefer to work on a project for just a matter of days or hours. ' + ('The following statements are about how long a typical project takes before ' + 'you consider it complete, and how large the codebase is when it is finished.'));
	return A3($author$project$Data$Likert$fivePointScale, title, description, statements);
}();
var $author$project$Main$init = function (flags) {
	return _Utils_Tuple2(
		{
			scales: $elm$core$Dict$fromList(
				_List_fromArray(
					[
						_Utils_Tuple2('The extent to which the end-goal is defined.', $author$project$Main$scaleOne),
						_Utils_Tuple2('The extent of programming effort, over time, and in the size of the resulting codebase', $author$project$Main$scaleTwo)
					]))
		},
		$elm$core$Platform$Cmd$none);
};
...
var $author$project$Data$Likert$rate = F3(
	function (statement, value, likertScale) {
		var updateIf = F2(
			function (predicate, f) {
				return $elm$core$List$map(
					function (a) {
						return predicate(a) ? f(a) : a;
					});
			});
		var ratingValues = A2($elm$core$List$map, $elm$core$Tuple$first, likertScale.ratings);
		var rateWithDefault = F2(
			function (rating, range) {
				return A2($elm$core$List$member, rating, range) ? rating : (($elm$core$List$sum(range) / 2) | 0);
			});
		var statements = A3(
			updateIf,
			A2(
				...
				$elm$core$Basics$eq(statement)),
			$elm$core$Tuple$mapFirst(
				$elm$core$Basics$always(
					A2(rateWithDefault, value, ratingValues))),
			likertScale.statements);
		return _Utils_update(
			likertScale,
			...
			_Utils_update(
				model,
				{
					scales: A3(
						...
						key,
						$elm$core$Maybe$map(
							A2($author$project$Data$Likert$rate, statement, rating)),
						model.scales)
				}),
			$elm$core$Platform$Cmd$none);
	});
var $author$project$Main$CheckItem = F3(
	function (a, b, c) {
		return {$: 'CheckItem', a: a, b: b, c: c};
	});
var $elm$html$Html$main_ = _VirtualDom_node('main');
var $elm$html$Html$h2 = _VirtualDom_node('h2');
var $elm$html$Html$section = _VirtualDom_node('section');
var $elm$html$Html$br = _VirtualDom_node('br');
var $author$project$Data$Likert$viewLabels = function (ratings) {
	return A2(
		$elm$html$Html$li,
		_List_fromArray(
			[
			...
			A2(
				...
				function (_v0) {
					...
					return A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('inline-block text-center')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text(label)
...
var $author$project$Data$Likert$viewStatement = F3(
	function (handler, ratings, _v0) {
		var currentRating = _v0.a;
		var statement = _v0.b;
		var radios = A2(
			$elm$core$List$map,
			A2(
							...
							[
								...
								_List_fromArray(
									[
										...
										_Utils_eq(r, currentRating)),
										$elm$html$Html$Attributes$class('flex justify-center'),
										$elm$html$Html$Events$onClick(
										A2(handler, statement, r))
									]),
								_List_Nil)
							]));
				}),
		...
		return A2(
			...
			A2(
				...
				A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('mr-8')
						]),
					_List_fromArray(
						[
...
var $author$project$Data$Likert$toHtml = F2(
	function (handler, likert) {
		var statements = A2(
			$elm$core$List$map,
			A2($author$project$Data$Likert$viewStatement, handler, likert.ratings),
			likert.statements);
		var labels = $author$project$Data$Likert$viewLabels(likert.ratings);
		return A2(
			$elm$html$Html$section,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('bg-gray-200 m-4 px-10 py-4 rounded-lg')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('border-b-2 border-black mb-2 text-2xl text-bold')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(likert.title)
						])),
					A2(
					$elm$html$Html$p,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('mb-4 text-justify')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(likert.description)
						])),
					A2(
					$elm$html$Html$ul,
					_List_fromArray(
						[
							...
							'likert-scale-' + A2($elm$core$Basics$composeL, $elm$core$String$fromInt, $elm$core$List$length)(likert.ratings))
						]),
					A2($elm$core$List$cons, labels, statements))
				]));
	});
	...
	return A2(
		...
		A2(
			...
			function (_v0) {
				...
				var scale = _v0.b;
				return A2(
					$author$project$Data$Likert$toHtml,
					$author$project$Main$CheckItem(k),
					scale);
			},
			$elm$core$Dict$toList(model.scales)));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{init: $author$project$Main$init, subscriptions: $author$project$Main$subscriptions, update: $author$project$Main$update, view: $author$project$Main$view});
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{},"unions":{"Main.Msg":{"args":[],"tags":{"CheckItem":["String.String","String.String","Basics.Int"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"String.String":{"args":[],"tags":{"String":[]}}}}})}});
